...
<body>
  ...
  <main class="results">
    <header class="results-hero">
      ...
      <h1>ViT-CRNN Results</h1>
      <p class="subtitle">Recognition results for the proposed ViT-CRNN pipeline on the Combined Test Set.</p>
    </header>
    ...
    <section class="results-stats">
      ...
      <div class="stat-card">
        <div class="stat-label">Lexicon Corrections</div>
        <div class="stat-value stat-lex">
          {{ collect($results)->filter(fn($r) => isset($r['predicted_label_lex']) && $r['predicted_label_lex'] !== $r['predicted_label'])->count() }}
        </div>
      </div>
    </section>
    ...
    <section class="results-grid">
      @foreach($results as $result)
        @php
          ...
          $hasLexCorrection = isset($result['predicted_label_lex']) && $result['predicted_label_lex'] !== $result['predicted_label'];
        ...
        <article class="result-card {{ $isCorrect ? 'correct' : 'incorrect' }}">
          ...
          <div class="card-content">
            ...
            @if($hasLexCorrection)
              <div class="card-row lex-row">
                <div class="card-label">Lexicon Corrected</div>
                <div class="card-value lex-correction">{{ $result['predicted_label_lex'] }}</div>
              </div>
            @endif
          </div>
        </article>
